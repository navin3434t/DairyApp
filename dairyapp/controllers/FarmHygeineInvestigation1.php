...
class FarmHygeineInvestigation1 extends Front_Controller {
		...
		if(isset($input) && !empty($input))
		{
			if( (!empty($input['user_id']) && isset($input['user_id'])) && (!empty($headerStringValue['webservice_token'])) && ( isset($input['investigationStep1'])) && ( isset($input['investigationStep2'])) && ( isset($input['investigationStep3'])) && ( isset($input['investigationStep4'])) && ( isset($input['investigationStep5'])) && ( isset($input['investigationStep6'])) && ( isset($input['investigationStep7'])) && ( isset($input['investigationStep8'])) && ( isset($input['investigationStep9'])) && ( isset($input['investigationStep10'])) && ( isset($input['bulkMilkTankCleaning'])) && ( isset($input['completeInvestigation'])) && ( isset($input['investigationRecommendation1'])) && ( isset($input['investigationRecommendation2'])) && ( isset($input['investigationRecommendation3'])) && (isset($input['investigationRecommendation4'])) && (!empty($input['local_db_id']) && isset($input['local_db_id'])) )
			{
				...
				if($validate['is_valid']==1)
				{
					...
					$bulkMilkTankCleaning=$input['bulkMilkTankCleaning'];			$completeInvestigation=$input['completeInvestigation'];			$investigationRecommendation1=$input['investigationRecommendation1'];
						...
						if($response["is_insert"]=="1")
						{
							...
							if(!empty($investigationRecommendation2))
							{
								...
								if(!empty($invite_data_s14)){
									$this->sendShareEmail2($invite_data_s14,$milking_machine_data_s14,$machine_wash_data_s14);
								}
							...
							if(!empty($investigationRecommendation3))
							{
								...
								{
									$this->sendShareEmail3($invite_data_s15,$user_id,$fhi_id);
								}
							}
							...
							if(!empty($investigationRecommendation4))
							{
								...
								if(!empty($action_data_s14)){
									$response=$this->FHI->investigationRecommendation4offline($action_data_s14,$fhi_id,$user_id);
								}	
							}
...
public function sendShareEmail2($invite_data,$milking_machine_data_s14,$machine_wash_data_s14)
	{
		
			ini_set('memory_limit', '40M');
			// load library
			$data=array();
			$data['machine_data']=$milking_machine_data_s14;
			$data['wash_data']=$machine_wash_data_s14;

			// boost the memory limit if it's low ;)
			$html = $this->load->view('investigation2/pdfmail', $data, true);
			...
			$pdfFilePath = $this->config->item("DIR_ROOT_IMAGE")."upload/attach/pdf_name2.pdf";
			$pdf->WriteHTML($html);
...
$config = array(
	...
	public function sendShareEmail3($invite_data,$recommend3_data_s15,$cycle_arr_s15)
	{
		
			ini_set('memory_limit', '40M');
			// load library
			$data=array();
			$data['recommend']=$recommend3_data_s15;
			$data['cycle']=$cycle_arr_s15;
			...
			$pdfFilePath = $this->config->item("DIR_ROOT_IMAGE")."upload/attach/pdf_name3.pdf";
			$pdf->WriteHTML($html);
			$pdf->Output($pdfFilePath, "F");
			$this->load->library("email");	
		// $this->email->clear(TRUE);
