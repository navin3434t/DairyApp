...
class CoolingPerformance extends Front_Controller {
	...
	* @return           : json data
	********************************************/
	public function createCoolingPerformanceDummy()
	{ 
		...
		if(isset($input) && !empty($input))
		{
			...
			if( (!empty($input['user_id']) && isset($input['user_id'])) && (!empty($headerStringValue['webservice_token'])) && (!empty($input['farm_id']) && isset($input['farm_id'])) && (!empty($input['input_json'])) && isset($input['action_json'])  && isset($input['local_db_id']) && (!empty($input['status']) && isset($input['status'])) )
			{
				...
				if($validate['is_valid']==1)
				{
					...
					$data_insert=array('input_json'=>$input_json,'action_json'=>$action_json,'local_db_id'=>$local_db_id,'farm_id'=>$farm_id,'user_id'=>$user_id,'create_date'=>date('Y-m-d'),'status'=>$status);
					/* Store data  */
					$response=$this->CPM->createCoolingPerformanceDummy($data_insert,$local_db_id);
	...
	/***********************************************************
	* Method Name   	: getActionsListDummy
	* Description       : Get Actions List
	* @Param            : user_id,webservice_token
	* @return           : json data
	********************************************/
	public function getActionsListDummy()
	{
		$headerStringValue = apache_request_headers();
		$inputJSON = file_get_contents('php://input');
		$input = json_decode($inputJSON, TRUE); //convert JSON into array		
		if(isset($input) && !empty($input))
		{
			if( (!empty($input['farm_id']) && isset($input['farm_id'])) && (!empty($input['user_id']) && isset($input['user_id'])) && (!empty($headerStringValue['webservice_token'])) )
			{
				...
				$validate = $this->UM->validateWebservice($validate_data);
				if($validate['is_valid']==1){
					$response= $this->CPM->getActionsListDummy($farm_id,$user_id);
					if($response)
					{	
	...
	public function getActionsList()
	{
		...
		if(isset($_POST) && !empty($_POST))
		{
			if( (!empty($_POST['user_id']) && isset($_POST['user_id'])) && (!empty($headerStringValue['webservice_token'])) )
			{
				...
				$validate = $this->UM->validateWebservice($validate_data);
				if($validate['is_valid']==1){
					$response['open']= $this->FM->getActionsListName('da_cooling_performance',$user_id,'0','da_invite_cooling_performace','cooling_performance');
					$response['close'] = $this->FM->getActionsListName('da_cooling_performance',$user_id,'1','da_invite_cooling_performace','cooling_performance');
					if(!empty($response))
					{	
						echo json_encode(array("status"=>"1","msg"=>"Data Found","data"=>$response));
					}
