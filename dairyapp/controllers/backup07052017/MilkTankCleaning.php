	...
	public function getActionsList()
	{
		...
		if(isset($_POST) && !empty($_POST))
		{
			if( (!empty($_POST['user_id']) && isset($_POST['user_id'])) && (!empty($headerStringValue['webservice_token'])) )
			{
				...
				$validate = $this->UM->validateWebservice($validate_data);
				if($validate['is_valid']==1){
					$response['open']= $this->FM->getActionsListProgramName('da_milk_tank_cleaning',$user_id,'0','da_invite_milking_machine_cleaning');
					$response['close'] = $this->FM->getActionsListProgramName('da_milk_tank_cleaning',$user_id,'1','da_invite_milking_machine_cleaning');
					if(!empty($response))
					{	
