	...
	public function getActionsList()
	{
		...
		if(isset($_POST) && !empty($_POST))
		{
			if( (!empty($_POST['user_id']) && isset($_POST['user_id'])) && (!empty($headerStringValue['webservice_token'])) )
			{
				...
				$validate = $this->UM->validateWebservice($validate_data);
				if($validate['is_valid']==1){
					$response['open'] = $this->FM->getActionsListName('da_cip_cleaning_routine',$user_id,'0','da_invite_cip_cleaning_routine');
					
					$response['close'] = $this->FM->getActionsListName('da_cip_cleaning_routine',$user_id,'1','da_invite_cip_cleaning_routine');
					if(!empty($response))
					{	
