	...
	public function getActionsList()
	{
		...
		if(isset($_POST) && !empty($_POST))
		{
			if( (!empty($_POST['user_id']) && isset($_POST['user_id'])) && (!empty($headerStringValue['webservice_token'])) )
			{
				...
				$validate = $this->UM->validateWebservice($validate_data);
				if($validate['is_valid']==1){
					$response['open']["cip_cleaning_routine"] = $this->FM->getActionsListName('da_cip_cleaning_routine',$user_id,'0','da_invite_cip_cleaning_routine');
					$response['open']["cooling_performance"] = $this->FM->getActionsListName('da_cooling_performance',$user_id,'0','da_invite_cooling_performace');
					$response['open']["farm_hygiene_investigation"] = $this->FM->getActionsListName('da_farm_hygiene_investigation',$user_id,'0','da_invite_hygiene_investigation');
					$response['open']["milking_machine_cleaning"] = $this->FM->getActionsListProgramName('da_milking_machine_cleaning',$user_id,'0','da_invite_milking_machine_cleaning');
					$response['open']["milk_tank_cleaning"] = $this->FM->getActionsListProgramName('da_milk_tank_cleaning',$user_id,'0','da_invite_milk_tank_cleaning');
					
					$response['close']["cip_cleaning_routine"] = $this->FM->getActionsListName('da_cip_cleaning_routine',$user_id,'1','da_invite_cip_cleaning_routine');
					$response['close']["cooling_performance"] = $this->FM->getActionsListName('da_cooling_performance',$user_id,'1','da_invite_cooling_performace');
					$response['close']["farm_hygiene_investigation"] = $this->FM->getActionsListName('da_farm_hygiene_investigation',$user_id,'1','da_invite_hygiene_investigation');
					$response['close']["milking_machine_cleaning"] = $this->FM->getActionsListProgramName('da_milking_machine_cleaning',$user_id,'1','da_invite_milking_machine_cleaning');
					$response['close']["milk_tank_cleaning"] = $this->FM->getActionsListProgramName('da_milk_tank_cleaning',$user_id,'1','da_invite_milk_tank_cleaning');
					if(!empty($response))
					{	
