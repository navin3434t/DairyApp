	...
	class FarmHygeineInvestigation_Model extends CI_Model
	{
		...
		********************************************/
		function investigationStep4offline($data_arr,$fhi_id,$user_id,$local_db_id)
		{
			/* Delete   */
			$this->db->where('local_db_id',$local_db_id);
			$this->db->delete('da_fhi_equipment_inspection');
		...
		function investigationStep5offline($data_assessment,$data_cycle,$fhi_id,$user_id)
		{
			...
			if($insert)
			{
				...
				foreach($data_cycle as $data)
				{
					...
					$pass=(isset($data['pass'])?$data['pass']:"");
					$fail=(isset($data['fail'])?$data['fail']:"");
					$data_insert=array('description'=>$description,'volume'=>$volume,'cleaner_sanitiser'=>$cleaner_sanitiser,'start_temp'=>$start_temp,'comments'=>$comments,'dose'=>$dose,'pass'=>$pass,'fail'=>$fail,'fhiwac_id'=>$insert_id,'local_db_id'=>$local_db_id);
					$this->db->insert("da_fhi_wash_assessment_cycle",$data_insert);
				}
				$log_arr=array('user_id'=>$user_id,'activity_type'=>"Create Hygeine Investigation Step4",'table_name'=>'da_fhi_wash_assessment','table_id'=>$insert_id,'date_time'=>date('Y-m-d h:i:s'));
				$this->saveActivityLog($log_arr);
		...
		function investigationStep9offline($data_arr,$data_arr_unit,$fhi_id,$user_id)
		{
			...
			{	
				...
				foreach($data_arr_unit as $data)
				{
					...
					$flow_rate=(isset($data['flow_rate'])?$data['flow_rate']:"");
					$pass_fail=(isset($data['pass_fail'])?$data['pass_fail']:"");
					$data_insert=array('unit_no'=>$unit_no,'letter_line'=>$letter_line,'volume_collected'=>$volume_collected,'time_elapsed'=>$time_elapsed,'flow_rate'=>$flow_rate,'pass_fail'=>$pass_fail,'fhi_flow_rate_id'=>$insert_id,'local_db_id'=>$local_db_id);
					$this->db->insert("da_fhi_flow_rate_cycle",$data_insert);
				}
				$log_arr=array('user_id'=>$user_id,'activity_type'=>"Create Hygeine Investigation Step4",'table_name'=>'da_fhi_flow_rate','table_id'=>$insert_id,'date_time'=>date('Y-m-d h:i:s'));
				$this->saveActivityLog($log_arr);
		...
		* @return           : json data
		********************************************/
		function investigationStep10offline($milk_tank_arr,$milk_cycle_arr,$fhi_id,$user_id)
		{
			$local_db_id = $milk_tank_arr['local_db_id'];
			/* Delete   */
			$this->db->where('local_db_id',$local_db_id);
			$this->db->delete('da_fhi_program_assessment');
			/*Insert*/
			$insert = $this->db->insert("da_fhi_program_assessment",$milk_tank_arr);
			$insert_id = $this->db->insert_id();
			if($insert)
			{
				...
				foreach($milk_cycle_arr as $cycle_arr)
				{
					...
					$fail=(isset($cycle_arr['fail'])?$cycle_arr['fail']:"");
					$milk_tank=(isset($cycle_arr['milk_tank'])?$cycle_arr['milk_tank']:"");
					$cycle_data=array('fhi_pa'=>$insert_id,'cycle_description'=>$cycling_description,'volume'=>$volume,'cleanser_sanitiser'=>$cleanser_sanitiser,'temp_start'=>$temp_start,'comments'=>$comment,'dose'=>$dose,'pass'=>$pass,'fail'=>$fail,'milk_tank'=>$milk_tank,'local_db_id'=>$local_db_id);
					$this->db->insert("da_fhi_program_assessment_cycle",$cycle_data);
				}
				$log_arr=array('user_id'=>$user_id,'activity_type'=>"Create Hygeine Investigation Step4",'table_name'=>'da_fhi_program_assessment','table_id'=>$fhi_id,'date_time'=>date('Y-m-d h:i:s'));
				$this->saveActivityLog($log_arr);
		...
		********************************************/
		public function investigationRecommendation1offline($action_data,$fhi_id,$user_id,$local_db_id)
		{
			...
			foreach($action_data as $action_arr)
			{
				...
				$recommendation1_data=array('fhi_id'=>$fhi_id,'action_name'=>$action_name,'assign_to'=>$assign_to,'completion_date'=>$completion_date,"create_date"=>date('Y-m-d'),"created_by"=>$user_id,'local_db_id'=>$local_db_id);
				$this->db->insert("da_fhi_recommendation1",$recommendation1_data);
				$insert_id = $this->db->insert_id();
				/*Insert Recommendation1 invite data*/
				$recommendation1_invite_data = array('user_id'=>$assign_to,'table_id'=>$insert_id,'invited_by'=>$user_id);
				...
				foreach($machine_wash_data as $wash_detail){
					...
					if($insert_days){
						...
						foreach($cycles as $cycle){
							...
							$dose=$cycle['dose'];
							$temp_dump=$cycle['temp_dump'];
							$cycle_data = array("fhi_recommendation2_days_id"=>$machine_wash_days_id,"cycle_description"=>$description,"volume"=>$volume,"cleaner_sanitiser"=>$cleaner_sanitiser,"comments"=>$comments,"temp_start"=>$temp_start,"dose"=>$dose,"temp_dump"=>$temp_dump,'local_db_id'=>$local_db_id);
							$insert_cycle = $this->db->insert("da_fhi_recommendation2_cycle",$cycle_data);
						}
					}				
				}
		...
		* @return           : json data
		********************************************/
		function investigationRecommendation3offline($recommend3_data,$cycle_arr,$fhi_id,$user_id){
			...
			if($insert)
			{
				...
				foreach($cycle_arr as $cycle){	
					...
					$dose=$cycle['dose'];
					$temp_dump = $cycle['temp_dump'];
					$cycle_data = array("fhi_recommendation3_id"=>$insert_id,"cycle_description"=>$description,"volume"=>$volume,"cleaner_sanitiser"=>$cleaner_sanitiser,"comments"=>$comments,"temp_start"=>$temp_start,"dose"=>$dose,"temp_dump"=>$temp_dump,'local_db_id'=>$local_db_id);
					$insert_cycle = $this->db->insert("da_fhi_recommendation3_cycle",$cycle_data);
		...
		* @return           : json data
		********************************************/
		function getInvestigationList($farm_id)
		{
			$this->db->select("name,create_date");
			$this->db->from("da_farm_hygiene_investigation");
