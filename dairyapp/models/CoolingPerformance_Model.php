...
/***********************************************************
		...
		* @return           : json data
		********************************************/
		function createCoolingPerformanceDummy($data_insert,$local_db_id)
		{
			$this->db->select("*");
			$this->db->from("da_coolling_performance_dummy");
			...
			if($query->num_rows()>0)
			{
				...
				$this->db->where('local_db_id',$local_db_id);
				$update = $this->db->update("da_coolling_performance_dummy",$data_insert);
				if($update)
				{
					$log_arr=array('user_id'=>$data_insert['user_id'],'activity_type'=>"Create Cooling Performance",'table_name'=>'da_cooling_performance','table_id'=>$insert_id,'date_time'=>date('Y-m-d h:i:s'));
			...
			{
				$insert = $this->db->insert("da_coolling_performance_dummy",$data_insert);
				$insert_id = $this->db->insert_id();
				if($insert)
				{
					$log_arr=array('user_id'=>$data_insert['user_id'],'activity_type'=>"Create Cooling Performance",'table_name'=>'da_coolling_performance_dummy','table_id'=>$insert_id,'date_time'=>date('Y-m-d h:i:s'));
					$this->saveActivityLog($log_arr);
					/* Save Precooling info */
		...
		* @return           : json data
		********************************************/
		function getActionsListDummy($farm_id,$user_id)
		{

			$this->db->select("user_id,farm_id,input_json,action_json,local_db_id,status");
			$this->db->from("da_coolling_performance_dummy");
			...
			if($query->num_rows()>0)
			{
				...
				{
					$result_new[$cnt]=array('user_id'=>$result->user_id,'farm_id'=>$result->farm_id,'input_json'=>$result->input_json,'action_json'=>$result->action_json,'local_db_id'=>$result->local_db_id,'status'=>$result->status);
					$cnt++;
				}
				return $result_new;
			}
