<?php
	class Farm_Model extends CI_Model
	{
		...
		* @return           : json data
		********************************************/
		function getFarmDataInvited($farm_id,$user_id){
			$this->db->select("a.id as farm_id,a.name as farm_name,a.address as farm_adderess,a.post_code as farm_post_code,a.gps_cordinates as farm_gps_cordinates,a.primary_contact,a.user_id,a.secondary_contact,a.status as complete_status,a.create_date as farm_create_date");
			$this->db->from("da_farm as a");
			$this->db->join("da_farm_invite as b","b.farm_id=a.id");
			$where_cond = array('b.farm_id'=>$farm_id,"b.user_id"=>$user_id,"b.status"=>"1");
			$this->db->where($where_cond);
			$query = $this->db->get();
			if($query->num_rows()>0)
			{
...
/***********************************************************
		* Method Name   	: getFarmNameByID
		* Description       : Get Invited Users
		* @Param            : user_id,webservice_token,farm_id contains
		* @return           : json data
		********************************************/
		function getFarmNameByID($fhi_id){
			$this->db->select("b.id as farm_id,b.name as farm_name");
			$this->db->from("da_farm_hygiene_investigation as a");
			$this->db->join("da_farm as b","b.id=a.farm_id");
			$where_cond = array('a.id'=>$fhi_id);
			$this->db->where($where_cond);
			$query = $this->db->get();
			if($query->num_rows()>0)
			{
				$result = $query->result();
				$farm_name = "Investigation - ".$result[0]->farm_name;
			}
...
/***********************************************************
		...
		********************************************/
		function getFarmList($user_id){
			$this->db->select("a.id as farm_id,a.name as farm_name,a.address as farm_address,a.post_code as farm_post_code,a.gps_cordinates as farm_gps_cordinates,a.status as complete_status,a.create_date as farm_create_date,count(d.user_id) as user_count,a.user_id,a.primary_contact,a.secondary_contact");
			...
			if($query->num_rows()>0)
			{
				...
				{
					$farm_id=$farmlist->farm_id;
					$farm_user=$this->getFarmUsersByID($farmlist->user_id);
					/*$userCount=$this->getFarmListInvitedUserCount($farmlist->farm_id); */
					$primary_user=$this->getFarmUsersByID($farmlist->primary_contact);
					...
					{
						$primary_user_arr=array();	
					}
					$secondary_user=$this->getFarmUsersByID($farmlist->secondary_contact);
				...
				if(!empty($result_invite_arr))
				{
					foreach($result_invite_arr as $farmlist)
					{
						...
						$response_farm_users = $this->getFarmUsers($farmlist->farm_id);						
						$result_new_arr[]=array('farm_id'=>$farmlist->farm_id,'farm_name'=>$farmlist->farm_name,'farm_address'=>$farmlist->farm_address,'farm_post_code'=>$farmlist->farm_post_code,'farm_gps_cordinates'=>$farmlist->farm_gps_cordinates,'complete_status'=>$farmlist->complete_status,'create_date'=>$farmlist->farm_create_date,'user_count'=>$userCount,'user_details'=>array('id'=>$farm_user[0]->id,'first_name'=>$farm_user[0]->first_name,'last_name'=>$farm_user[0]->last_name,'email'=>$farm_user[0]->email,'email'=>$farm_user[0]->email),'primary_contact'=>$primary_user_arr,'secondary_contact'=>$secondary_user_arr,'farm_users'=>$response_farm_users);
		...
		* @return           : json data
		********************************************/
		function getFarmListInvitedUserCount($farm_id)
		{
			$this->db->select("a.*");
		...
		* @return           : json data
		********************************************/
		function getActionsListInvetigation($table,$user_id,$close_status,$table_invite){
			$this->db->select("a.id as action_id,a.action_name as action_name,a.create_date as create_date,a.task_status as complete_status,a.assign_id as user_id");
			$this->db->from("$table as a");
			$where_cond = array("a.assign_id"=>$user_id,"a.close_status"=>$close_status);
			/* $this->db->join("da_user as c","c.id=a.user_id"); */
			$this->db->where($where_cond);
			$query = $this->db->get();
			$result_new_arr=array();			
			if($query->num_rows()>0)
			{
				...
				$result_new_arr_invited=$this->getActionsListNameInvitedInvestigation($table,$user_id,$close_status,$table_invite);
		...
		* @return           : json data
		********************************************/
		function getActionsListName($table,$user_id,$close_status,$table_invite,$action_type){
			...
			if($query->num_rows()>0)
			{
				...
				if(!empty($result_new_arr_invited))
				{
					...
					{
						$action_users=$this->getFarmUsersByID($action_list->user_id);
						if($table=="da_farm_hygiene_investigation")
						{
							$action_details=$this->FHI->getInvestigationDetails($action_list->sub_module_id);
							//$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'sub_module_id'=>$action_list->sub_module_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type,'action_details'=>$action_details);
							$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'sub_module_id'=>$action_list->sub_module_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type);
						}
						else
						{
		...
		* @return           : json data
		********************************************/ 
		function getActionsListNameInvitedInvestigation($table,$user_id,$close_status,$table_invite){
			$this->db->select("a.id as action_id,a.fhi_id as fhi_id,a.action_name as action_name,a.create_date as create_date,a.task_status as complete_status,a.assign_id as user_id");
			$this->db->from("$table as a");
			$where_cond = array("c.user_id"=>$user_id,"a.close_status"=>$close_status);
		...
		********************************************/
		function getActionsListNameInvited($table,$user_id,$close_status,$table_invite){
			$this->db->select("a.id as action_id,a.id as sub_module_id,a.farm_id as farm_id,a.name as action_name,a.create_date as create_date,a.complete_status as complete_status,a.user_id");
			$this->db->from("$table as a");
		...
		* @return           : json data
		********************************************/
		function getActionsListProgramName($table,$user_id,$close_status,$table_invite,$action_type){
		...
		/***********************************************************
		* Method Name   	: getActionsListProgramNameInvited
		* Description       : Get Actions List Program Name Invited
		* @Param            : user_id,webservice_token
		* @return           : json data
		********************************************/
		function getActionsListProgramNameInvited($table,$user_id,$close_status,$table_invite){
			$this->db->select("a.id as action_id,a.id as sub_module_id,a.farm_id as farm_id,a.program_name as action_name,a.create_date as create_date,a.complete_status as complete_status,a.user_id");
			$this->db->from("$table as a");
			$where_cond = array("c.user_id"=>$user_id,"a.close_status"=>$close_status,'c.status'=>"1");
		...
		/***********************************************************
		* Method Name   	: getActionsListActionName
		* Description       : Get Action List Action Name
		* @Param            : user_id,webservice_token
		* @return           : json data
		********************************************/
		function getActionsListActionName($table,$user_id,$close_status,$table_invite,$action_type){
			$this->db->select("a.id as action_id,a.action_name as action_name,a.create_date as create_date,a.completion_status as complete_status,a.assign_id");
			$this->db->from("$table as a");
			$where_cond = array("a.assign_id"=>$user_id,"a.close_status"=>$close_status);
			/* $this->db->join("da_user as c","c.id=a.user_id");*/
			$this->db->where($where_cond);
			...
			if($query->num_rows()>0)
			{
				...
				$result_new_arr_invited=$this->getActionsListActionNameInvited($table,$user_id,$close_status,$table_invite);
				/* foreach($result as $action_list)
				{
					$action_users=$this->getFarmUsersByID($action_list->assign_id);
					$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users);
				} */
				/* Invited Action List */
				if(!empty($result_new_arr_invited))
				{
					...
					{
						$action_users=$this->getFarmUsersByID($action_list->assign_id);
						if($table=="da_fhi_recommendation1")
						{
							$sub_module_id=$action_list->sub_module_id;
						}
						else
						{
							$sub_module_id=$action_list->action_id;
						}
						$action_details=$this->FHI->getInvestigationDetails($sub_module_id);
						//$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type,'sub_module_id'=>$sub_module_id,'action_details'=>$action_details);
						$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type);
					}					
				}				
				$response = $result_new_arr;				
			}
				...
				if(!empty($result_new_arr_invited))
				{
					foreach($result_new_arr_invited as $action_list)
					{
						...
						$action_details=$this->FHI->getInvestigationDetails($sub_module_id);
						$action_users=$this->getFarmUsersByID($action_list->assign_id);
						//$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type,'action_details'=>$action_details);
						$result_new_arr['action_details'][]=array('action_id'=>$action_list->action_id,'farm_id'=>$action_list->farm_id,'action_name'=>$action_list->action_name,'create_date'=>$action_list->create_date,'complete_status'=>$action_list->complete_status,'user_details'=>$action_users,"action_type"=>$action_type);
					}	
					$response = $result_new_arr;
				}				
		...
		/***********************************************************
		* Method Name   	: getActionsListActionNameInvited
		* Description       : Get Actions List Action Name Invited
		* @Param            : user_id,webservice_token
		* @return           : json data
		********************************************/
		function getActionsListActionNameInvited($table,$user_id,$close_status,$table_invite)
		{
			if($table=="da_fhi_recommendation1")
			{
				$this->db->select("a.id as action_id,e.id as farm_id,a.fhi_id as sub_module_id,a.action_name as action_name,a.create_date as create_date,a.completion_status as complete_status,a.assign_id");	
				$this->db->from("$table as a");
				$where_cond = array("c.user_id"=>$user_id,"a.close_status"=>$close_status,'c.status'=>"1");
				
				$this->db->join("$table_invite as c","c.table_id=a.id");				
				$this->db->join("da_farm_hygiene_investigation as d","a.fhi_id=d.id");				
				$this->db->join("da_farm as e","d.farm_id=e.id");				
				
			}
			else
			{
				$this->db->select("a.id as action_id,a.farm_id as farm_id,a.id as sub_module_id,a.action_name as action_name,a.create_date as create_date,a.completion_status as complete_status,a.assign_id");
				$this->db->from("$table as a");
				$where_cond = array("c.user_id"=>$user_id,"a.close_status"=>$close_status,'c.status'=>"1");
